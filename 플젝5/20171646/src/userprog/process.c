...
process_execute (const char *file_name) 
{
  ...
  command = strtok_r(command, " ", &ptr);

  tid = thread_create(command, PRI_DEFAULT, start_process, tmp);


  if(tid == TID_ERROR)
	goto execute_failed;

  int cur_idx = search_wait_list(thread_current()->tid);
  wait_list[cur_idx].load_flag = 0;

  while(1){
	if(wait_list[cur_idx].load_flag != 0)
		break;
	thread_yield();
  }
  ...
  for(int i = 0; i < 60; i++){
	...
	if(wait_list[i].load_flag == -1 && wait_list[i].parent_tid == thread_current()->tid){
  ...
  execute_failed:
	if(tmp) palloc_free_page(tmp);
	if(command) palloc_free_page(command);
	return -1;
}
...
start_process (void *file_name_)
{
  ...
  t = thread_current();
  cur_idx = search_wait_list(t->tid);


  //palloc_free_page (file_name);
  int p_idx = search_wait_list(wait_list[cur_idx].parent_tid);
  wait_list[p_idx].load_flag = 1;

  if (!success){
	wait_list[cur_idx].load_flag = -1;
	exit(-1);
  }
...
#ifdef VM
  success = success && page_table_install(t->page_table, upage, kpage, -1, NULL, 0, 0, 0, false, FRAME_PAGE);
#endif
  return success;
}
