...
{
  off_t base = 0, limit = 0;
  block_sector_t ret;

  limit += 1 * DIRECT_BLOCKS_COUNT;
  base = limit;
  ...
  if (index < limit) 
    ...
    limit += 1 * INDIRECT_BLOCKS_PER_SECTOR;
    ...
    limit += 1 * INDIRECT_BLOCKS_PER_SECTOR * INDIRECT_BLOCKS_PER_SECTOR;
    ...
    if (index < limit) {
      ...
      buffer_cache_read (indirect_idisk->blocks[first], indirect_idisk);
      ret = indirect_idisk->blocks[second];
                ...
                off_t offset)
{
  ...
  if( byte_to_sector(inode, offset + size - 1) == -1u ) {
    res = inode_reserve (& inode->data, offset + size);
    if (res == false) 
	return 0;
...
inode_reserve_indirect (block_sector_t* p_entry, size_t num_sectors, int level)
{
  ...
  if (level == 0) {
    if (*p_entry == 0) {
      if(free_map_allocate (1, p_entry) == false)
        return false;
