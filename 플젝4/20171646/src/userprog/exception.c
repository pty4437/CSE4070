...
page_fault (struct intr_frame *f)
{
  ...
  bool stack_on_frame = false, stack_is_addr = false;

  
  if(fault_addr == f->esp-32 || fault_addr == f->esp-4 || esp <= fault_addr)
	stack_on_frame = true;
  if(fault_addr < PHYS_BASE && PHYS_BASE <= fault_addr + MAX_STACK_SIZE)
	stack_is_addr = true;
  

  if (stack_on_frame && stack_is_addr) {

    pt_entry *pte = page_table_lookup(curr->page_table, fault_page);

    if(pte == NULL)
	page_table_install(curr->page_table, fault_page, NULL, 0, NULL, 0, 0, 0, false, ZERO_PAGE);
  }

  if(!load_page(curr->page_table, curr->pagedir, fault_page) ) {
    goto PAGE_FAULT_VIOLATED_ACCESS;
  }
